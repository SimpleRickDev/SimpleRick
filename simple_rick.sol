...
/*
   SimpleRick features:
   15% Supply burned immediately
   1% fee auto add to the liquidity pool to locked forever when selling
   1% fee for a charity the community decides on
   2% reflected back to $SRICK holders
...
contract SimpleRick is Context, IERC20, Ownable {
    ...
    string private _name = "Simple Rick";
    string private _symbol = "SRICK";
    uint8 private _decimals = 18;

    uint256 private constant MAX = ~uint256(0);
    // total supply 2 billion
    uint256 private _tTotal = 20000000000.000 * 10 ** uint256(_decimals);
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    uint256 public _maxTxAmount = 20000000000.0000 * 10 ** uint256(_decimals);
    // max to liquify is 0.05% of total
    ...
    uint256 private _charityPercentageOfLiquidity = 100;
    ...
    function transferCharity() public onlyCharity
    {
    ...
    function setupFeeDefaults() external onlyOwner {
        _taxFee = 1;
        _liquidityFee = 1;
        _maxTxAmount = _tTotal.mul(100).div(10**2);
    }

    // Allow setting tx fee for safety purposes
    // Never allow max fee percent to go above 1% Also for safety purposes
    function setTaxFeePercent(uint256 taxFee) external onlyOwner() {
        require(taxFee <= 2, "Cannot set tax fee percent greater than 2% (should be 1%)");
        require(taxFee >= 0, "Cannot set negative tax fee because what does that even mean");
        _taxFee = taxFee;
    }

    // similar to the above. we don't want to lock out liquidity so allow to set it but never more than 50%
    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner() {
        require(liquidityFee <= 50, "Cannot set liq fee percent greater than 50%");
        require(liquidityFee >= 0, "Cannot set negative liq fee because that's bad");
        _liquidityFee = liquidityFee;
    }

    //  allows flexibility if needed to break up any trading log jams. Probably won't be used, but it's safe to use if we need it
    function setCharityPercentageOfLiquidity(uint256 charityPercent) external onlyOwner() {
        require(charityPercent >= 1, "Cannot set charity fee percent lower than 1%");
        require(charityPercent <= 100, "Cannot set charity fee greater than 100 because duh");
        _charityPercentageOfLiquidity = charityPercent;
    }

    // Allow setting max tx percent for safety purposes
    // Never allow max tx percent to fall below 10%. Also for safety purposes
    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner() {
        require(maxTxPercent >= 10, "Cannot set transaction amount less than 10 percent!");
        require(maxTxPercent <= 100, "Cannot set transaction amount greater than 100% because what even");
